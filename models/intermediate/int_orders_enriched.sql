...
order_attribution as (
    select order_item_id, attributed_traffic_source
    from (
        select
            oi.order_item_id,
        ...
        left join purchase_events pe
            ...
            {% else %}
            and pe.event_created_at between oi.order_item_created_at - interval '1 hour'
                                        and oi.order_item_created_at + interval '1 hour'
            {% endif %}
    )
    where rn = 1
),
final as (
    select
        oi.order_item_id, oi.order_id, oi.user_id, oi.product_id, oi.inventory_item_id,
        oi.order_item_status, oi.order_item_created_at, oi.order_item_shipped_at,
        oi.order_item_delivered_at, oi.order_item_returned_at, oi.sale_price,
        p.product_name, p.category, p.brand, p.department, p.cost, p.retail_price,
    ...
    left join order_attribution oa on oi.order_item_id = oa.order_item_id
)
select * from final
